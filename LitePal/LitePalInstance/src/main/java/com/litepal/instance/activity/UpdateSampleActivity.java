		...
		switch (v.getId())
		{
			case R.id.update_btn1:
				try
				{
					...
					int rowsAffected = singer.update(Long.parseLong(mSingerIdEdit.getText().toString()));
			...
			case R.id.update_btn2:
				try
				{
					...
					singer.setName(mSingerNameEdit.getText().toString());
					singer.setAge(Integer.parseInt(mSingerAgeEdit.getText().toString()));
					int rowsAffected = singer.updateAll("name=? and age=?", mNameToUpdateEdit.getText().toString(), mAgeToUpdateEdit.getText().toString());
					Toast.makeText(this, String.format(getString(R.string.number_of_rows_affected), String.valueOf(rowsAffected)), Toast.LENGTH_SHORT).show();
					populateDataFromDB();
				}
				catch (Exception e)
