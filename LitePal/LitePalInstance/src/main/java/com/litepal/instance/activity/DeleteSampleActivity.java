...
package com.litepal.instance.activity;
...
import com.litepal.instance.R;
import com.litepal.instance.adapter.DataArrayAdapter;
import com.litepal.instance.model.Singer;
	...
	public void onClick(View v)
	{
		switch (v.getId())
		{
			case R.id.delete_btn1:
				try
				{
					int rowsAffected = DataSupport.delete(Singer.class, Long.parseLong(mSingerIdEdit.getText().toString()));
					Toast.makeText(this, String.format(getString(R.string.number_of_rows_affected), String.valueOf(rowsAffected)), Toast.LENGTH_SHORT).show();
					populateDataFromDB();
				}
				catch (Exception e)
				{
					e.printStackTrace();
					Toast.makeText(this, getString(R.string.error_param_is_not_valid), Toast.LENGTH_SHORT).show();
				}
				break;
			case R.id.delete_btn2:
				try
				{
					int rowsAffected = DataSupport.deleteAll(Singer.class, "name=? and age=?", mNameToDeleteEdit.getText().toString(), mAgeToDeleteEdit.getText().toString());
					Toast.makeText(this, String.format(getString(R.string.number_of_rows_affected), String.valueOf(rowsAffected)), Toast.LENGTH_SHORT).show();
					populateDataFromDB();
				}
				catch (Exception e)
