...
public class QueryClusterTest extends LitePalTestCase
{
	...
	public void testWhere()
	{
		...
		List<Book> expectedBooks = getBooks(null, "bookname like ?", new String[]{"Android%Line"}, null, null, null, null);
		List<Book> realBooks = DataSupport.where("bookname like ?", "Android%Line").find(Book.class);
	...
	public void testCluster()
	{
		...
		List<Book> books = DataSupport.select("pages", "isPublished").where("id=? or id=? or id=?", String.valueOf(ids[0]), String.valueOf(ids[1]), String.valueOf(ids[2])).order("id").limit(2).offset(1).find(Book.class);
		Book firstBook = DataSupport.select("pages", "isPublished").where("id=? or id=? or id=?", String.valueOf(ids[0]), String.valueOf(ids[1]), String.valueOf(ids[2])).order("id").limit(2).offset(1).findFirst(Book.class);
		Book lastBook = DataSupport.select("pages", "isPublished").where("id=? or id=? or id=?", String.valueOf(ids[0]), String.valueOf(ids[1]), String.valueOf(ids[2])).order("id").limit(2).offset(1).findLast(Book.class);
