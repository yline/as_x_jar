...
public class Many2OneUniSaveTest extends LitePalTestCase
{
	...
	public void testSaveFast()
	{
		...
		Assert.assertFalse(isFKInsertCorrect(getTableName(c1), getTableName(t1), c1.get_id(), t1.getId()));
		Assert.assertFalse(isFKInsertCorrect(getTableName(c1), getTableName(t2), c1.get_id(), t2.getId()));
	}

	private void assertFK(Classroom c1, Teacher t1, Teacher t2)
	{
		Assert.assertTrue(isFKInsertCorrect(getTableName(c1), getTableName(t1), c1.get_id(), t1.getId()));
		Assert.assertTrue(isFKInsertCorrect(getTableName(c1), getTableName(t2), c1.get_id(), t2.getId()));
	}
