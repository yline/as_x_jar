package com.litepal.instance.test;
...
import com.litepal.instance.model.Computer;
...
public class LitePalTestCase extends InstrumentationTestCase
{
	...
	/**
	 * @param table1   Table without foreign key.
	 * @param table2   Table with foreign key.
	 * @param table1Id id of table1.
	 * @param table2Id id of table2.
	...
	protected boolean isFKInsertCorrect(String table1, String table2, long table1Id, long table2Id)
	{
		...
		try
		{
			cursor = db.query(table2, null, "id = ?", new String[]{String.valueOf(table2Id)}, null, null, null);
			cursor.moveToFirst();
			long fkId = cursor.getLong(cursor.getColumnIndexOrThrow(BaseUtility.changeCase(table1 + "_id")));
	...
	protected boolean isIntermediateDataCorrect(String table1, String table2, long table1Id, long table2Id)
	{
		SQLiteDatabase db = Connector.getDatabase();
		Cursor cursor = null;
		try
		{
			...
			cursor = db.query(DBUtility.getIntermediateTableName(table1, table2), null, where, new String[]{String.valueOf(table1Id), String.valueOf(table2Id)}, null, null, null);
	...
	{
		Cursor cursor = Connector.getDatabase().query(tableWithFK, null, "id = ?", new String[]{String.valueOf(id)}, null, null, null);
		long foreignKeyId = 0;
		if (cursor.moveToFirst())
		{
			foreignKeyId = cursor.getLong(cursor.getColumnIndexOrThrow(BaseUtility.changeCase(tableWithoutFK + "_id")));
		}
		cursor.close();
		return foreignKeyId;
	}
	...
	protected List<Book> getBooks(String[] columns, String selection, String[] selectionArgs, String groupBy, String having, String orderBy, String limit)
	{
		List<Book> books = new ArrayList<Book>();
		Cursor cursor = Connector.getDatabase().query(getTableName(Book.class), columns, selection, selectionArgs, groupBy, having, orderBy, limit);
		if (cursor.moveToFirst())
		{
			do
			{
